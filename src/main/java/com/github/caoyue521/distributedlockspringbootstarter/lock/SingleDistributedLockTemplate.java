...
public class SingleDistributedLockTemplate implements DistributedLockTemplate {
    ...
    public <T> T lock(DistributedLockCallback<T> callback) {
        RLock lock = getLock(callback.getLockName(),false);
        try{
            lock.lock();
    ...
    public <T> T tryLock(DistributedLockCallback<T> callback, long waitTime, long leaseTime, TimeUnit timeUnit, boolean fairLock) {
        ...
        try {
            if (lock.tryLock(waitTime, leaseTime, timeUnit)) {
                ...
                log.error("wait lock out time ,lockName :[{}] ",callback.getLockName());
                throw  new DistributedLockOutWaitTimeException("wait lock out time");
            }
        } catch (InterruptedException e) {
             log.error("try lock  InterruptedException happened ,lockName :[{}] ",callback.getLockName());
             throw  new DistributedLockRuntimeException(e.getMessage());
        } finally {

            if (lock != null && lock.isLocked()) {
                try {
                    lock.unlock();
                    log.info("unlock success,lockName is [{}]",lock.getName());
                }catch (IllegalMonitorStateException e){
                    log.warn("unlock failed,lockName is [{}]",lock.getName());
                }
